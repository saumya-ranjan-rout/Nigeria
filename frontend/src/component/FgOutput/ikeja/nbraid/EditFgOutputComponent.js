...
export function EditFgOutputComponent() {
...
const handleDateChange = (date) => {
  ...
  const handleSubmit = (event) => {
    ...
    $.ajax({
      url: 'http://192.168.29.243:4000/fg_output_update',
      method: 'POST',
      data: jsonData,
      processData: false,
...
const saveFormData = (formData) => {
 ...
 useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      $.ajax({
        url: `http://192.168.29.243:4000/getfgoutputData/${id}`, // Replace with your API endpoint URL
        method: 'GET',
        success: function (response) {
          const { product_name, color_description, code, line, date, hour, fgoutput, shift, product_code } = response;
...
export default EditFgOutputComponent;
