...
export function LineMasterComponent() {
  ...
  const handleEdit = (id) => {
    history.push(`/master/edit-line/${id}`);
  };

  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Do You Really Want To Delete line??');
    if (confirmDelete) {
...
// Destroy the DataTable instance before making the fetch request
    if (tableRef.current) {
      ...
      fetch(`http://192.168.29.243:4000/linedelete/${id}`, {
        method: 'DELETE',
      })
        ...
        .then((data) => {
          ...
          setServerMessage('Line deleted successfully');
        ...
        .catch((error) => {
          ...
          setServerMessage('An error occurred while deleting line');
  ...
  const fetchData = () => {
    $.ajax({
      url: 'http://192.168.29.243:4000/getline',
      method: 'GET',
      success: function (response) {
        // Set the fetched data in the state
        setLineCategories(response);
  ...
  return (
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <h5 className="title">Line Master{' '}
            <Link to="/master/add-line" className="btn btn-success btn-sm rounded">
              Add new
            </Link>
            </h5>
        </div>
...
export default LineMasterComponent;
