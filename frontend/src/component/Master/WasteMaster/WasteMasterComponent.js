...
export function WasteMasterComponent() {
  ...
  const handleEdit = (id) => {
    history.push(`/master/edit-waste/${id}`);
  };

  const handleDelete = (id) => {
    const confirmDelete = window.confirm('Do You Really Want To Delete waste??');
    if (confirmDelete) {
    ...
    if (tableRef.current) {
      ...
      fetch(`http://192.168.29.243:4000/wastedelete/${id}`, {
        method: 'DELETE',
      })
        .then((response) => response.json())
        .then((data) => {
          console.log('Waste deleted:', data);
          // Refresh the list of items
          fetchData();
          // Set the server message and style it
          setServerMessage('Waste deleted successfully');
        ...
        .catch((error) => {
          ...
          setServerMessage('An error occurred while deleting waste');
  ...
  const fetchData = () => {
    $.ajax({
      url: 'http://192.168.29.243:4000/getwaste',
      method: 'GET',
      success: function (response) {
        // Set the fetched data in the state
        setWasteCategories(response);
  ...
  return (
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <h5 className="title">Waste Master{' '}
            <Link to="/master/add-waste" className="btn btn-success btn-sm rounded">
              Add new
            </Link>
            </h5>
        </div>
...
export default WasteMasterComponent;
