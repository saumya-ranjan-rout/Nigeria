...
export function MachineMasterComponent() {
  ...
  const handleEdit = (id) => {
    history.push(`/master/edit-machine/${id}`);
  };
  ...
  const handleDelete = (id) => {
    ...
    if (tableRef.current) {
      ...
      fetch(`http://192.168.29.243:4000/machinedelete/${id}`, {
        method: 'DELETE',
      })
  ...
  const fetchData = () => {
    $.ajax({
      url: 'http://192.168.29.243:4000/getmachine',
      method: 'GET',
      success: function (response) {
        // Set the fetched data in the state
        setMachineCategories(response);
          ...
          buttons: [
            {
              ...
              text: 'Copy',
              exportOptions: {
                columns: [0, 1, 2], // Include only the first and second columns in copying
              },
            },
            {
              extend: 'csv',
              text: 'CSV',
              exportOptions: {
                columns: [0, 1, 2], // Include only the first and second columns in CSV
              },
  ...
  return (
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <h5 className="title">Machine Master{' '}
            <Link to="/master/add-machine" className="btn btn-success btn-sm rounded">
              Add new
            </Link>
            </h5>
        </div>
...
export default MachineMasterComponent;
