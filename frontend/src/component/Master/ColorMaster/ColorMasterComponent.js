...
export function ColorMasterComponent() {
  ...
  const handleEdit = (id) => {
    history.push(`/master/edit-color/${id}`);
  };
  ...
  const handleDelete = (id) => {
    ...
    if (tableRef.current) {
      ...
      fetch(`http://192.168.29.243:4000/colordelete/${id}`, {
        method: 'DELETE',
      })
        ...
        .then((data) => {
          ...
          setServerMessage('Color deleted successfully');
  ...
  const fetchData = () => {
    $.ajax({
      url: 'http://192.168.29.243:4000/getcolor',
      method: 'GET',
  ...
  return (
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <h5 className="title">Color Master{' '}
            <Link to="/master/add-color" className="btn btn-success btn-sm rounded">
              Add new
            </Link>
            </h5>
        </div>
...
export default ColorMasterComponent;
