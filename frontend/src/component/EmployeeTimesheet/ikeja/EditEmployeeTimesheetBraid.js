...
export function EditEmployeeTimesheetBraid() {
...
const handleProductChange = async (e) => {
  ...
  try {
    const response = await fetch(`http://192.168.29.243:4000/getcolordescription/${selectedProduct}`);
...
const handleSubmitnew = (event) => {
  ...
  $.ajax({
    url: 'http://192.168.29.243:4000/updateemployeetimesheetfilterdata',
    type: 'POST',
...
useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      $.ajax({
        url: `http://192.168.29.243:4000/operator_edit/${id}`, // Replace with your API endpoint URL
        method: 'GET',
        success: function (response) {
...
const waste_weight = comparison.waste_weight;
        ...
        setFormData({item, site, shift, zone, machine, color_description, hr_start, hr_end, fiber, fg_output, waste1, waste2, waste3, waste_weight, upper, lower, perheating, machine_speed, tension, spreading, update_time });
...
// Destroy the existing DataTable instance (if it exists)
  ...
  tableRef.current = $('#example').DataTable({
    ...
    const fetchOperator = (empid, site) => {
      $.ajax({
        url: `http://192.168.29.243:4000/getOperator/${empid}/${site}`, // Use emp_id in the URL
        method: 'GET',
        success: function (response) {
          //alert(JSON.stringify(response));
...
setOpIdValue(opid);
  ...
  $.ajax({
    url: `http://192.168.29.243:4000/getcolordescriptionss/${item}`,
    method: 'GET',
    success: function (response) {
...
getcolordescriptionss();
  ...
  $.ajax({
    url: `http://192.168.29.243:4000/operatorsection/${id}`,
    method: 'GET',
    success: function (response) {
      const { rows } = response;
      ...
      <section id="content">
        ...
        <div className={serverMessageClass} style={{ padding: '5px', margin: '5px 0' }}>
          {serverMessage}
            ...
            <h5 className="title">Employee Details ({dateValue})
            </h5>
            <hr></hr>
