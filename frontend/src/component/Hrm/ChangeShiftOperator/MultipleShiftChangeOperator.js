...
export function MultipleShiftChangeOperator() {
  ...
  const handleSubmit = (event) => {
    ...
    //return;
    $.ajax({
      url: `${config.apiUrl}/operator/update-shift`,
      method: "POST",
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      ...
      //const entryIdsString = entryIdsArray.map(value => `"${value}"`).join(',');
      //alert(entryIdsString);
      $.ajax({
        url: `${config.apiUrl}/operator/operators/${entryIdsArray}`,
        method: 'GET',
        headers: customHeaders,
        processData: false,
        contentType: 'application/json',
        success: function (response) {
          ...
          const entryIds = timesheetData.map((item) => `${item.entryid}[${item.name}]`).join(',');
