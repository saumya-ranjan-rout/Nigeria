...
export function AddAssignOperator() {
  ...
  const history = useHistory();
    ...
    if (!isLoggedIn) {
      ...
      const fetchZone = () => {
        $.ajax({
          url: `http://192.168.29.243:4000/ikeja/getzone`,
          method: 'GET',
          success: function (response) {
            setZone(response);
 ...
 //get user id
  ...
  useEffect(() => {
    if (userId && userId.id) {
      $.ajax({
        url: `http://192.168.29.243:4000/ikeja/getItems/${userId.id}`,
        method: 'GET',
        success: function (response) {
          setItems(response);
        },
...
const handleSubmit = (event) => {
  ...
  $.ajax({
    url: 'http://192.168.29.243:4000/ikeja/add_zone',
    method: 'POST',
    data: jsonData,
    processData: false,
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      $.ajax({
        url: `http://192.168.29.243:4000/ikeja/item_zone_delete/${id}`,
        method: 'DELETE',
        success: function (response) {
          alert(response);
          window.location.reload();
