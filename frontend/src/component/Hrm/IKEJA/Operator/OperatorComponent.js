...
export function OperatorComponent() {
 ...
 const ConvertToCategory = (id) => {
  history.push(`/hrm/ikeja/convert_category_op/${id}`);
}
  const handleView = (id) => {
    history.push(`/hrm/ikeja/viewoperator/${id}`);
  };

  const assignOperator = (id) => {
    history.push(`/hrm/ikeja/addassignoperator/${id}`);
  };

  const assignOp_ndraid = (id) => {
    history.push(`/hrm/ikeja/addassign_nbraid/${id}`);
  };
  ...
  const DeleteOperator = (id) => {
    ...
    if (confirmDelete) {
        $.ajax({
        url: `http://192.168.29.243:4000/ikeja/operator_delete/${id}`,
        method: 'DELETE',
  ...
  const fetchData = () => {
    $.ajax({
      url: 'http://192.168.29.243:4000/ikeja/operator_data_view',
      method: 'GET',
      success: function (response) {
            ...
            },
            {
              ...
              render: function (data, type, row, meta) {
                ...
                <span style="display:inline;">
                  <button class="btn btn-sm btn-`+btn+`" style="color:#fff;font-size:13px;padding:auto;margin-bottom:2px;width :30px" onclick="`+lnk+`" title="Add"><i class="bx bx-plus-medical"></i></button>
                  ...
                  <button class="btn btn-sm btn-danger" style="color:#fff;font-size:15px;padding:2px 3px;margin-bottom:2px;width :30px" onclick="DeleteOperator('${eid}')" title="Delete"><i class="bx bx-trash-alt"></i></button>
                </span>  
              `
              },
            },
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <h5 className="title">Operator{' '}
            <Link to="/hrm/ikeja/addoperator" className="btn btn-success btn-sm rounded">
              Add new
            </Link>
            </h5>
        </div>
