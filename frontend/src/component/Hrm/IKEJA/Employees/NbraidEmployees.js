...
export function Employees() {
  ...
  const handleView = (id) => {
    history.push(`/hrm/ikeja/viewemployeenbraid/${id}`);
  };
...
const [formData, setFormData] = useState({
  ...
  const handleSubmit = (event) => {
    ...
    $.ajax({
        url: `http://192.168.29.243:4000/ikeja/get_employees_nbraid?where=${where}`,
        method: 'GET',
    })
    .done((response) => {
  ...
  useEffect(() => {
    ...
    fetchLines();
       //Default employees data fetch
        const fetchData = () => {
        $.ajax({
            url: 'http://192.168.29.243:4000/ikeja/get_employeess_nbraid',
            method: 'GET',
            success: function (response) {
            // Set the fetched data in the state
            setEmployees(response);
