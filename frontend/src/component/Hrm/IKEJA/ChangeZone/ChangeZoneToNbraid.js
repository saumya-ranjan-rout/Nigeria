...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      history.push('/login');
        ...
        const fetchOperatorData = () => {
            fetch('http://192.168.29.243:4000/ikeja/getZonedata')
 ...
 const handleZoneChange = (e) => {
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ikeja/multiplezonetonbraid?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `http://192.168.29.243:4000/ikeja/get_zonedata_search?where=${encodedWhere}`,
      method: 'GET',
  })
  .done((response) => {
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <main>
          ...
          <hr/>
          <p style={{color:'red',fontWeight:'bold'}}>All the employees are working on <span style={{color:'green'}}>BRAID</span> category.Here you can convert employees from braid to <span style={{color:'blue'}}>NBRAID</span></p>
          <br/>
          <div className='row'>
