...
export function MultipleChangeZoneToNbraid() {
  ...
  useEffect(() => {
    document.title = 'Change Braid To Non-braid For Multiple Employee';
 ...
 const handleSubmit = (event) => {
  ...
  if (isValid) {
  $.ajax({
    url: 'http://192.168.29.243:4000/ikeja/update_multiple_zone_to_nbraid',
    method: "POST",
    data: jsonData,
    processData: false,
    contentType: 'application/json',
...
const selectedOption1 = lines.find((data) => data.id === formData.line);
...
export default MultipleChangeZoneToNbraid;
