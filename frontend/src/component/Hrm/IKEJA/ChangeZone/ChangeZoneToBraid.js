...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      history.push('/login');
        ...
        const fetchOperatorData = () => {
            fetch('http://192.168.29.243:4000/ikeja/getZonedataNbraid')
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ikeja/multiplezoneBraid?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  const section = formData.section_id;
...
const encodedWhere = encodeURIComponent(where);
  $.ajax({
      url: `http://192.168.29.243:4000/ikeja/get_zoneNbraid_search?where=${encodedWhere}`,
      method: 'GET',
  })
  .done((response) => {
...
const selectedOption = sectionName.find((data) => data.id === formData.section_id);
