...
tableRef.current = $('#changezone').DataTable({
 ...
 const handleZoneChange = (e) => {
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ikeja/multiplechangezone?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  let where = `(geopos_employees.roleid != '3' AND geopos_employees.roleid !='5' AND geopos_employees.passive_type='ACT' AND geopos_employees.category_type='BRAID')`;
