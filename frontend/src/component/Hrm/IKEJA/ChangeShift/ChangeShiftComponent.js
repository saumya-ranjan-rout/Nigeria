...
tableRef.current = $('#changezone').DataTable({
  ...
  columns: [
      ...
      { data: 'role' },
      {
        ...
        render: function (data, type, row) {
            const workertype = data && data.workertype ? data.workertype : '';
            const shift = data && data.shift ? data.shift : '';
            return `${workertype} (${shift})`;
        },
    },
    {
        data: null,
        render: function (data, type, row) {
            const formattedMachineNames = data && data.machine ? formatMachineNames(data.machine) : '';
            const zone = data && data.zone ? data.zone : '';
            const sectionName = data && data.section_name ? data.section_name : '';
    
            return (
                `<span style="color:blue;font-weight:bold;">${zone}</span>` +
                `<br><span style="font-size:12px;">(${formattedMachineNames})` +
                `<br><b>Section:</b> <span style="color:green;">${sectionName}</span></span>`
            );
        },
    },
    ...
    if (!isLoggedIn) {
      ...
      history.push('/login');
        ...
        const fetchOperatorData = () => {
            fetch('http://192.168.29.243:4000/ikeja/getShiftdata')
 ...
 const handleZoneChange = (e) => {
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ikeja/multiplechangeshift?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `http://192.168.29.243:4000/ikeja/get_shiftdata_search?where=${encodedWhere}`,
      method: 'GET',
  })
  .done((response) => {
    ...
    <Sidebar />
      ...
      <Header />
        ...
        <main>
          ...
          <h5 className="title">Change Shift</h5>
          <br/>
