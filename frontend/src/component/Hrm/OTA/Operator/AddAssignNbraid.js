...
export function AddAssignOperatorNbraid() {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (userId && userId.id) {
      $.ajax({
        url: `http://192.168.29.243:4000/ota/getSectionitem/${userId.id}`,
        method: 'GET',
        success: function (response) {
          setItems(response);
        },
...
const handleSubmit = (event) => {
  ...
  $.ajax({
    url: 'http://192.168.29.243:4000/ota/addSectionOp',
    method: 'POST',
    data: jsonData,
    processData: false,
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      $.ajax({
        url: `http://192.168.29.243:4000/ota/delete_section_assign/${id}`,
        method: 'DELETE',
        success: function (response) {
          alert(response);
          window.location.reload();
...
const handleSearchChange = (event) => {
  ...
  const handleMove = () => {
    ...
    const id=userId.id;
    const redirectURL = `/hrm/ota/multiplesectionassign/${id}/${ids}`;
      ...
      <section id="content">
        ...
        <main>
          <div className="container dt">
            ...
            <form method='POST'>
              <div className="row space">
                <div className="col-sm-6">
                  <span className="textgreen">Choose Line , Section and Shift*</span>
                  <Select
                      ...
                      options={items.map((item) => ({
                        value: item.id,
                        label: `${item.line}  (${item.section_name}) (${item.shift})`,
                      }))}
                      value={selectedItems}
                      ...
                      isSearchable
                      placeholder="Choose Line, Sec, Shift"
                    />
                </div>
            ...
            <table id="" className="table borderd">
                <thead>
                  <tr>
                    <th>slno</th>
                    <th>line</th>
                ...
                {items.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item.shift}</td>
                      <td>
                        {/* Include your action button or content here */}
                        <button
