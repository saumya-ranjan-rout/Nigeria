...
export function BraidAssignList() {
  ...
  if (!isLoggedIn) {
    ...
    const fetchUsers = () => {
      $.ajax({
        url: `http://192.168.29.243:4000/ota/get_operator_assign_list`,
        method: 'GET',
        success: function (response) {
          setAssignList(response);
           // Destroy the existing DataTable instance (if it exists)
        if ($.fn.DataTable.isDataTable('#asssignbraid')) {
          $('#asssignbraid').DataTable().destroy();
        }
        // Initialize the DataTable with the updated data
        tableRef.current = $('#asssignbraid').DataTable({
          dom: 'Bfrtip',
          buttons: [
              {
                  extend: 'copy',
              ...
              {
                  extend: 'csv',
                  filename: 'Assign_Braid_list_Details', // Removed space in filename
                  exportOptions: {
                      columns: ':not(.action-column)',
                  },
              },
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      $.ajax({
        url: `http://192.168.29.243:4000/ota/delete_assign_list/${id}`,
        method: 'DELETE',
        success: function (response) {
          alert(response);
          ...
          <div className="container dt">
            ...
            <hr/>
             <div className='row'>
                ...
                <table id="asssignbraid" className="display">
                        <thead>
                            <tr>
                                <th>Slno</th>
                                <th>Name</th>
