...
tableRef.current = $('#changezone').DataTable({
    ...
    if (!isLoggedIn) {
      ...
      history.push('/login');
        ...
        const fetchOperatorData = () => {
            fetch('http://192.168.29.243:4000/ota/getZonedata')
 ...
 const handleZoneChange = (e) => {
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ota/multiplezonetonbraid?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `http://192.168.29.243:4000/ota/get_zonedata_search?where=${encodedWhere}`,
      method: 'GET',
  })
  .done((response) => {
