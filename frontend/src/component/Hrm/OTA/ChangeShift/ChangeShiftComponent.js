...
tableRef.current = $('#changezone').DataTable({
  ...
  columns: [
      ...
      //       if(data.category_type=='NBRAID'){
      //         return ('('+data.line+')<br>Section : <span style="font-size:12px;color:green;">'+
      //             data.section_name+'</span>'
      //         );
      //       }else{
      //         return ('('+data.zone+')<br>('+formattedMachineNames+')Section : <span style="font-size:12px;color:green;">'+
      //         data.section_name+'</span>'
      //         );
      //       }
      ...
      // },
      {
        ...
        render: function (data, type, row) {
          ...
          if (data.category_type == 'NBRAID') {
            return (
              `(${line})<br>Section : <span style="font-size:12px;color:green;">${sectionName}</span>`
            );
          } else {
            return (
              `(${zone})<br>(${formattedMachineNames})Section : <span style="font-size:12px;color:green;">${sectionName}</span>`
            );
          }
        },
      }
    ...
    if (!isLoggedIn) {
      ...
      history.push('/login');
        ...
        const fetchOperatorData = () => {
            fetch('http://192.168.29.243:4000/ota/getShiftdata')
  ...
  const handleMove = (entryIds) => {
    ...
    history.push(`/hrm/ota/multiplechangeshift?${queryParams}`);
  };
...
const handleSubmit = (event) => {
  ...
  $.ajax({
      url: `http://192.168.29.243:4000/ota/get_shiftdata_search?where=${where}`,
      method: 'GET',
  })
  .done((response) => {
